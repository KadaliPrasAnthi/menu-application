...
/**<li className='item-card'>
   ...
   const CategoryTab=({tabDetails,onSelectTab,activeTab})=>{
      const{name,id,value}=tabDetails
      const activeTabstyle=(activeTab===value)&&"active-tab"
      return <li>
        <button className={`filter-tab ${activeTabstyle}`} onClick={()=>onSelectTab(id)}>{name}</button>
        </li>
   }
   const DietTab=({dietDetails,onSelectDiet,activeDiet})=>{
      const{type,value}=dietDetails
      const activeDietstyle=(activeDiet===value)&&"active-tab"
      return <li>
        <button className={`filter-tab ${activeDietstyle}`} onClick={()=>{onSelectDiet(value)}}>{type}</button>
        </li>
   }
   const ItemCard=({itemDetails,onClickItem})=>{
   ...
   return (<li className='item-card' onClick={()=>onClickItem(id)}>
...
const MainMenu=()=>{
  ...
  const [itemCount,setItemCount]=useState(initialMenuData.length);
  ...
  useEffect(()=>{
    ...
    // if(itemCategory!==''){
    //   filteredItems=filteredItems.filter(each=>(
    //     each.category===itemCategory
    //   ))
    // }

    // if(itemIsVeg!==''){
    //   filteredItems=filteredItems.filter(each=>(
    //     each.isVeg===itemIsVeg
    //   ))
    // }
  ...
  const onClickLogout=()=>{
    Cookies.remove('party-menu-token');
  ...
  /*const onClickSearch=()=>{
    console.log('search')
    const filtereditems=initialMenuData.filter(each=>(
      each.name.toLocaleLowerCase().includes(searchInput.toLocaleLowerCase())
    ))
  ...
  const onSelectTab=(id)=>{
    //console.log(id)
    const specificCategory=categoryList.find(each=>each.id===id)
    const value=specificCategory.value
    setCategory(value)
  ...
  return <div className='party-menu-container'>
    ...
    <section className='filters-container'>
      <div className='filter-container'>
        <h2>CATEGORY</h2>
        <ul className='filter-tabs-container'>
          {/*<li><button className='filter-tab all-tab'>All</button></li>
          <li><button className='filter-tab'>Starter</button></li>
          <li><button className='filter-tab'>Main</button></li>
          <li><button className='filter-tab'>Sides</button></li>
          <li><button className='filter-tab'>Desert</button></li>
        */}
        {categoryList.map(each=>(
          <CategoryTab key={each.id} tabDetails={each} onSelectTab={onSelectTab} activeTab={itemCategory}/>
        ))}
      ...
      <div className='filter-container'>
        ...
        <ul className='filter-tabs-container'>
          {/*<li><button className='filter-tab all-tab'>All</button></li>
          <li><button className='filter-tab'>Veg</button></li>
          <li><button className='filter-tab'>Non-Veg</button></li>
          */}
          {dietList.map(each=>(
            <DietTab key={each.id} dietDetails={each} onSelectDiet={onSelectDiet} activeDiet={itemIsVeg}/>
          ))}
        </ul>
      </div>
      <div className='search-conatiner'>
        <input type="search" placeholder='Search by name (e.g. chicken)' onChange={onChangeInput}/>
        <button>Search</button>
      </div>
    </section>
    {/*items card section */}
    <section className='items-card-section'>
      <p>{itemCount} items found</p>
      <ul className='item-cards-container'>
          {menuData.map(each=>(
            <ItemCard key={each.id} itemDetails={each} onClickItem={onClickItem}/>
          ))}
      </ul>
    </section>
    </div>
